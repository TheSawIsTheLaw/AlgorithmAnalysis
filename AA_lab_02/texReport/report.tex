...
Трудоёмкость заполнения массива $cols$: $f_{cols} = 5 + \frac{K}{2}(2 + 11 \cdot K)$, где $K$ - количество строк второй переданной матрицы, $L$ - количество столбцов второй переданной матрицы.

Трудоёмкость заполнения массива $rows$: $f_{rows} = 2 + N \cdot (5 + 11 \cdot \frac{K}{2})$, где $M$ - количество строк первой переданной матрицы, $K$ - количество столбцов первой переданной матрицы.

Трудоёмкость цикла заполнения итоговой матрицы для чётных размеров матриц: $f_{evencyc} = 2 + N \cdot (2 + L \cdot (5 + 2 + 22 \cdot \frac{K}{2}))$, где $N$ - количество строк первой матрицы, $L$ - количество столбцов второй матрицы, $K$ - количество строк второй матрицы.

$f_{evencyc} = 2 + 2 \cdot N + 7 \cdot N \cdot L + 11 \cdot N \cdot L \cdot K$.

Трудоёмкость цикла для дополнительного заполнения при нечётных размерах матриц: $f_{end} = 2 + M \cdot (2 + \frac{K}{2} \cdot 12)$. При этом рассматривается два случая, представленных в выражении \ref{end}:

\begin{equation}
	\label{end}
            f_{end} = \begin{cases}
                2, & \text{чётная,}\\
                4 + 22 \cdot M + 6 \cdot K \cdot M, & \text{иначе.}
            \end{cases}
        \end{equation}
...
\begin{equation}
	\label{bad}
	f = 6 + 2 \cdot N + 5 \cdot K + 22 \cdot M + 7 \cdot N \cdot L + 17 \cdot M \cdot K + 11 \cdot N \cdot L \cdot K
\end{equation}

Для лучшего случая, когда размеры матриц чёртные, из \ref{endAll}:
\begin{equation}
	\label{good}
	f = 4 + 2 \cdot N + 11 \cdot M + 7 \cdot N \cdot L + 11 \cdot N \cdot L \cdot K
\end{equation}
